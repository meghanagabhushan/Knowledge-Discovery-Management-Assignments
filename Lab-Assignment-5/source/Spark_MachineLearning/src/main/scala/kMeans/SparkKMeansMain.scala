...
object SparkKMeansMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val inputPath=Seq("data/combinedData/*")
    Logger.getRootLogger.setLevel(Level.WARN)

    val topic_output = new PrintStream("data/CombinedKmeansResult.txt")
    // Load documents, and prepare them for KMeans.
    val preprocessStart = System.nanoTime()
   // val (corpusVector, data, vocabSize) = preprocess(sc, inputPath)
   val (corpusVector, data, vocabSize) = preprocessWithoutNLP(sc, inputPath)
    ...
    val rdd = model.predict(corpusKM)

    val error = model.computeCost(corpusKM)
    val results = data.zip(rdd)
  ...
  private def preprocess(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], RDD[(String,String)], Long) = {
    ...
    //tfidf.saveAsTextFile("tfidfvectorsForFitness")
    val dff= stopWordRemovedDF.flatMap(f=>f._2)
    val vocab=dff.distinct().collect()
    tfidf.collect()
    (tfidf, data, dff.count()) // Vector, Data, total token count
  }
  private def preprocessWithoutNLP(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], RDD[(String,String)], Long) = {
    val dfmegha = sc.wholeTextFiles(paths.mkString(",")).map(f => {
      //val lemmatised=CoreNLP.returnLemma(f._2)
      val splitStringmegha = f._2.split(" ")
      (f._1,splitStringmegha)
    })
    val data=dfmegha.map(f=>{(f._1,f._2.mkString(" "))})
    val dfseqmegha=dfmegha.map(_._2.toSeq)

    //Creating an object of HashingTF Class
    val hashingTF = new HashingTF(dfmegha.count().toInt)  // VectorSize as the Size of the Vocab

    //Creating Term Frequency of the document
    val tfmegha = hashingTF.transform(dfseqmegha)

    tfmegha.cache()

    val iDFModel = new IDF().fit(tfmegha)
    //Creating Inverse Document Frequency
    val tfidf1megha = iDFModel.transform(tfmegha)
    tfidf1megha.cache()


    val tfidfmegha=tfidf1megha.zipWithIndex().map(_.swap)
   // tfidf.saveAsTextFile("tfidfvectorsWithoutNLP")
    val dff= dfmegha.flatMap(f=>f._2)
    val vocab=dff.distinct().collect()
    tfidfmegha.collect()
    (tfidfmegha, data, dff.count()) // Vector, Data, total token count
  }
}
