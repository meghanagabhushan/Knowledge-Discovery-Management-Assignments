...
object OntContructorMain {
  def main(args: Array[String]): Unit = {
    ...
    val ONTOLOGYURI="http://www.semanticweb.org/meghanagabhushan/ontologies/2017/6/"

    val manager = OWLManager.createOWLOntologyManager
    //creating ontology manager
    val df = manager.getOWLDataFactory //In order to create objects that represent entities

    val ontology = manager.createOntology(IRI.create(ONTOLOGYURI,"sqad#"))
    //Prefix for all the entities
    val pm = new DefaultPrefixManager(null, null, ONTOLOGYURI+"sqad#")


    // Declaration Axiom for creating Classes
    val classes=Source.fromFile("subjectFile").getLines()
...
/*    val subClasses=Source.fromFile("data/SubClasses").getLines()
    ...
    val os = new FileOutputStream("data/sqad.owl")
